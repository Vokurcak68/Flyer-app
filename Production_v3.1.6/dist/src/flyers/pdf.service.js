...
let PdfService = PdfService_1 = class PdfService {
    ...
    async convertImageToJPEG(imageBuffer, maxWidth = 800, quality = 100) {
        try {
    ...
    async convertIconToPNG(imageBuffer, maxWidth = 100, compressionLevel = 9) {
        try {
            const image = sharp(imageBuffer);
            const metadata = await image.metadata();
            if (metadata.width && metadata.width > maxWidth) {
    ...
    async generateFlyerPDF(flyer, userRole) {
    ...
    async renderSlotGrid(doc, page, flyer, userRole) {
        const startX = 0;
        const startY = 0;
        const pageWidth = 595;
    ...
    async renderProductSlot(doc, x, y, width, height, product, userRole) {
        const padding = 6;
        ...
        if (product.imageData) {
            try {
                ...
                const productQuality = parseInt(this.configService.get('PDF_PRODUCT_JPEG_QUALITY', '100'));
            ...
            try {
                ...
                for (let slotIndex = 0; slotIndex < maxSlots; slotIndex++) {
                    ...
                    if (icon.imageData) {
                        ...
                        const iconCompression = parseInt(this.configService.get('PDF_ICON_PNG_COMPRESSION', '9'));
    ...
    async renderPromoSlot(doc, x, y, width, height, promoImage, validFrom, validTo) {
        if (promoImage.imageData) {
            try {
                ...
                const promoQuality = parseInt(this.configService.get('PDF_PROMO_JPEG_QUALITY', '100'));
                ...
                if (promoImage.fillDate && validTo) {
                    ...
                    const textX = x + width - textWidth - 25;
